...
import { fetchCreateLandingLead, fetchUpdateLandingLead } from '@/service/api/xiaomai/landing-lead';
...
async function handleSubmit() {
  ...
  const { id, contactName, organizationName, contactEmail, subject, message, sourceLocale, processingStatus, remark } = model.value;

  // request
  if (props.operateType === 'add') {
    const { error } = await fetchCreateLandingLead({ contactName, organizationName, contactEmail, subject, message, sourceLocale, processingStatus, remark });
    if (error) return;
    window.$message?.success($t('common.addSuccess'));
  }

  if (props.operateType === 'edit') {
    const { error } = await fetchUpdateLandingLead({ id, contactName, organizationName, contactEmail, subject, message, sourceLocale, processingStatus, remark });
    if (error) return;
