...
- [ ] 建立共享 RuoYi Client 抽象（AC: 1, 3）
  - [ ] 设计统一的 `ruoyi_client` 入口、认证 / 请求头策略、超时与重试边界。
  - [ ] 区分单条响应 `{code, msg, data}` 与分页响应 `{code, msg, rows, total}` 的解包逻辑。
  - [ ] 约束 FastAPI feature 只依赖 Client 接口与 DTO，不直接依赖 RuoYi Java 领域对象。
- [ ] 定义 DTO、Mapper 与调用语义（AC: 1, 2）
  - [ ] 为写入、查询、分页查询和错误返回分别定义 DTO / mapper。
  - [ ] 在 mapper 层吸收 RuoYi 字段命名、状态枚举与日期格式差异。
  - [ ] 为视频、课堂、Companion、Evidence、Learning Coach 等场景设计可扩展的资源调用方法。
- [ ] 建立错误映射与日志策略（AC: 2, 3）
  - [ ] 将 401、403、404、409、5xx 与网络异常映射为可解释的集成错误。
  - [ ] 记录 request_id、task_id、目标资源、失败原因与重试结果。
  - [ ] 禁止把原始框架堆栈直接透出到业务层或前端。
- [ ] 补齐单元与集成测试（AC: 1, 2, 3）
  - [ ] 使用 mock HTTP 覆盖成功、分页、字段漂移与失败语义。
  - [ ] 覆盖回写失败、权限不足、目标资源不存在与超时重试场景。
  - [ ] 验证新增资源接入时只需扩展 mapper / DTO，而不是复制一套新 client。
...
- 已为 FastAPI 与 RuoYi 之间的统一防腐层客户端补齐抽象边界、错误映射与测试要求。
