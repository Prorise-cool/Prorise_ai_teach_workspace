...
- [ ] 定义统一 Provider Protocol（AC: 1, 3）
  - [ ] 定义 LLM / TTS 共享的基础能力接口与返回约束。
  - [ ] 明确 Provider 标识符、配置项与错误边界。
- [ ] 建立工厂与注册骨架（AC: 1, 2, 3）
  - [ ] 实现 `ProviderFactory` 与优先级注册表。
  - [ ] 支持按配置装配主备 Provider 列表。
- [ ] 提供 demo provider 与接入示例（AC: 2, 3）
  - [ ] 补齐最小 demo provider，演示注册、实例化与调用流程。
  - [ ] 明确业务层只能依赖 Protocol，而不是具体实现类。
- [ ] 建立工厂层测试（AC: 1, 2, 3）
  - [ ] 覆盖注册顺序、优先级排序、配置驱动装配与未知 Provider 错误。
  - [ ] 覆盖新增 Provider 不需要改业务调用代码的示例。
...
- 已将 Provider Story 收敛为协议定义、工厂装配、优先级注册与 demo provider 四个清晰执行面。
