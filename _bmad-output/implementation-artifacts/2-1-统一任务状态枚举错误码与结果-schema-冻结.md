...
- [ ] 冻结统一任务状态与状态机映射（AC: 1）
  - [ ] 定义对外五态与后端内部 `queued`、`running`、`retrying` 等状态的映射规则。
  - [ ] 明确取消、重试、失败收敛与“不允许悬挂态”的约束。
- [ ] 冻结任务结果、快照与事件 schema（AC: 1, 2）
  - [ ] 在 `contracts/tasks/` 下补齐 `TaskResult`、`TaskSnapshot`、`TaskProgressEvent` 等契约文档或 schema。
  - [ ] 提供成功、失败、恢复快照与 Provider 切换的示例 payload。
- [ ] 冻结错误码与追踪字段（AC: 2, 3）
  - [ ] 建立 `TaskErrorCode` 字典，并定义 `request_id`、`task_id`、`errorCode` 的追踪语义。
  - [ ] 明确前端展示层只消费稳定字段，不猜测 `message` 文案的结构。
- [ ] 建立 mock 与契约测试基线（AC: 1, 2, 3）
  - [ ] 在 `mocks/tasks/` 下提供最小生命周期、失败态与恢复态样例。
  - [ ] 为 schema 序列化与错误码映射增加单元测试或契约测试。
...
- 已补齐统一任务契约 Story 所需的字段冻结规则、命名冲突处理原则与资产落位建议。
