...
- [x] 明确机器可读契约消费边界（AC: 2, 3）
  - [x] 约定 `contracts/tasks/*.schema.json` 与 FastAPI `OpenAPI` 共同作为任务域机器可读资产来源。
  - [x] 约定前端继续复用单一 `services/api/client.ts`，类型或 adapter scaffold 可由 schema / OpenAPI 生成，但不得旁路共享 client。
- [x] 冻结错误码与追踪字段（AC: 2, 3）
...
- 对外公开的共享类型、adapter、运行时底座工具与协议模型，必须同步满足仓库的文件头、JSDoc / docstring 规范，不能只冻结字段不补说明。
...
- 已明确任务域的机器可读消费边界：`contracts/tasks/*.schema.json` 与 `FastAPI OpenAPI` 共同承接类型来源，前端继续复用单一 `Fetch API client`。
- 已通过以下验证：

### Post-Review Alignment Fixes

- 补充了“机器可读契约如何被前端消费”的明确边界，避免后续继续靠自然语言或聊天记录同步字段。
- 补充了共享底座源码与契约说明需遵守仓库规范化注释 / docstring 约束的说明。
