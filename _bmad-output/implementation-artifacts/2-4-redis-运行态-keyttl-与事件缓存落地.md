...
- [x] 落地统一 Redis key builder 与 TTL 常量（AC: 1）
  - [x] 实现 `xm_task`、`xm_task_events`、`xm_provider_health` 等 key 生成器。
  - [x] 固化 2h、1h、60s 等 TTL 规则，并禁止无过期时间写入。
- [x] 实现运行态快照与事件缓存封装（AC: 1, 2）
  - [x] 提供任务快照写入、最新状态读取、事件列表追加与恢复读取能力。
  - [x] 保证恢复逻辑读取的是“最小必要状态”，而不是依赖数据库历史重放。
- [x] 明确 Redis 与长期数据宿主边界（AC: 2, 3）
  - [x] 为视频、课堂、Companion、Provider 健康状态定义运行态与长期态分界说明。
  - [x] 防止业务开发把长期结果直接留在 Redis。
- [x] 补充运行态测试与清理策略（AC: 1, 2, 3）
  - [x] 覆盖 key 命名、TTL 生效、事件缓存读取与过期清理。
  - [x] 覆盖错误写法的保护，例如无 TTL 写入或把长期数据误写到运行态 namespace。
...
- `.venv/bin/python -m pytest tests/unit/task_framework/test_runtime_store.py tests/unit/task_framework/test_scheduler.py tests/unit/test_task_contracts.py tests/integration/test_dramatiq_broker.py tests/unit/test_task_trace.py tests/test_health.py tests/test_bootstrap_routes.py`

### Completion Notes List

- 已将 `RuntimeStore` 落地为强制 TTL 的运行态存储层，统一收口 `xm_task`、`xm_task_events`、`xm_task_message` 与 `xm_provider_health` 命名。
- 已让 `TaskScheduler` 在写入快照时同步落地结构化事件缓存，使 Worker 执行链路不依赖内存 broker 也能恢复事件序列。
- 已补齐 Redis 运行态边界与 key/TTL 契约文档，并提供 `task-events.progress` mock 供后续恢复逻辑复用。
- 已覆盖 key 命名、TTL、生存期过期、事件补发、恢复视图与长期数据边界保护测试。
