...
### 10.3.4 前端 i18n 与主题约定

\[Rule] 所有正式页面与共享组件的关键静态文案必须进入 i18n 资源管理，不允许以“先硬编码、后期再抽取”为默认开发路径。\
\[Rule] 所有正式页面、关键状态与共享组件必须消费统一语义化 Design Token，不允许通过直接写死颜色绕过亮 / 暗色主题体系。\
\[Rule] Frontend Story 的设计、实现、测试与验收均需同时覆盖默认语言、i18n 资源归属以及 light / dark 双模式下的关键状态。

| 类别 | 强制规则 |
|------|----------|
| 文案 | 面向用户的固定文案使用 i18n key，资源命名与默认语言归属在 Story 进入开发前明确 |
| 主题 | 颜色、边框、阴影、表面、文本对比度通过语义 token 组织；`light / dark` 都必须保持可读、可用、不破版 |
| 验收 | 页面 Story 至少验证默认语言渲染、关键文案资源命中、light / dark 关键状态可用性 |
| 禁止写法 | 禁止把 Agent 风格误当整站主题；禁止以后期集中返工替代前期接入 i18n / 主题基线 |
