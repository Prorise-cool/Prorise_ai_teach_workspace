...
| **前端 UI (FR-UI)** | 9 | landing、首页、课堂、视频输入/播放、学习中心域（含历史/收藏视图）、个人资料、设置、来源抽屉 / 证据面板、i18n / 亮暗色基线 | 组件复用 + 风格重写 |
...
| **Agent 风格 = 数据预设** | 不做按 Agent 切换的多主题系统；亮 / 暗色基础主题独立存在，差异在 AgentConfig | \[Priority]\[MVP] |
...
| **国际化** | 前端所有页面 | react-i18next 基线需从首个正式页面 Story 起接入；MVP 默认中文，完整多语内容可后续补齐 | \[Priority]\[MVP] |
| **亮 / 暗色主题** | 前端所有页面 | 统一语义化 Design Token、Light / Dark 双模式与关键状态兼容性必须同步建设 | \[Priority]\[MVP] |
...
| **统一品牌风格 + 基础亮暗色** | 所有页面共享小麦品牌设计系统，同时提供 light / dark 双模式；不做按 Agent 切换的多主题 | \[Priority]\[MVP] |
| **Agent 差异局部化** | 头像和点缀色差异仅体现在局部组件 | \[Priority]\[MVP] |
| **文案资源化前置** | 所有正式页面的关键静态文案从首个 Story 起进入 i18n 资源，而不是等后期集中返工 | \[Priority]\[MVP] |
