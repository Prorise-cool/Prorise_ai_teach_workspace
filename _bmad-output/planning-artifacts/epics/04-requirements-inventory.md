...
FR-UI-003: 视频生成页承载题目输入、图片上传、用户配置透传、任务创建与进度查看。
...
FR-UI-009: 营销落地页用于活动投放、赛事展示、试点说明与合作转化，不替代默认首页。
...
FR-VS-010: 用户可在视频输入页发现公开视频卡片，并从公开结果进入观看或复用生成。
FR-VP-001: 用户可以播放已生成的视频。
FR-VP-002: 用户可以切换视频播放速度。
FR-VP-003: 用户可拖动进度条并切换全屏。
FR-VP-004: 用户可在视频结果页将结果公开发布，并为视频输入页提供复用入口。
FR-CS-001: 用户输入一个主题后，系统可生成一套基础课堂内容。
FR-CS-002: 用户可配置个人简介和学习偏好，系统根据配置动态生成/匹配适配的 AI agents。
...
FR-CS-008: 课堂输入页支持可选联网搜索增强，让课堂生成在需要时参考公开资料与指定证据范围。
FR-CS-009: 用户可在课堂结果页导出课堂结果，供备课沉淀、分享或二次使用。
...
FR-KQ-007: Evidence / Retrieval 服务支持联网搜索与公开资料检索，为课堂生成或证据问答提供公开来源增强。
...
- AR-002: 前端仍需手动集成 `Zustand`、`Motion`、`KaTeX / Temml`、`Shiki`、HTTP 客户端、`react-router-dom`、`react-i18next`、`Video.js` 与 SSE 客户端工具。
...
- AR-023: 当前批准的前端成品图目录固定为 `docs/03UI:UX 设计素材/001UI 设计稿/04-成品图`，前端 Story 在实施时必须引用对应页面或共享状态成品图作为视觉与交互基线。
- AR-024: 若成品图未覆盖某个业务必需点、配置项或异常状态，仍必须按 PRD、UX 与 Epic / Story 完整实现；若成品图中出现且与既有业务边界不冲突的新增前台模块，也必须回填到对应 Epic / Story 后纳入实施范围。
...
UX-DR-006: 用户配置系统在首次使用时引导用户完成，配置页面包含昵称、个人简介（10-500字符）、学习风格多选和语言选择。
...
UX-DR-030: 组件库需至少覆盖双入口页面组件、用户配置组件、进度反馈组件、视频播放器组件、Companion 组件、Evidence / Retrieval 组件与 Learning Coach 组件。
