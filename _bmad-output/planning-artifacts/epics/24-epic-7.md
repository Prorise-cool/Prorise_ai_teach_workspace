...
- 与 `Epic 6` 并行时，`Epic 7` 只拥有资料依据抽屉、上传区、术语解释和来源历史，不拥有 Companion 侧栏。  
- 与 `Epic 5` 并行时，只允许对齐课堂输入页中的“联网搜索配置透传”，不允许在课堂主结果区直接抢占展示结构。  
- 未冻结来源抽屉挂载位前，不得让多个 Agent 同时改同一个结果页右侧容器。  
