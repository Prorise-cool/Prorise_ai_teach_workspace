...
- 任务 / SSE 相关 OpenAPI、JSON schema 与示例 payload 可作为机器可读资产稳定输出，前端不依赖自然语言猜字段；  
- 底座共享源码与契约说明遵守当前仓库的文件头、JSDoc / docstring 等规范化说明要求。  
...
**Given** 前端或测试需要消费统一任务契约  
**When** 团队查看机器可读资产  
**Then** 可以从 schema / OpenAPI / 示例 payload 获取稳定字段定义  
**And** 不需要通过聊天记录或口头说明手工抄写字段  
...
- 统一任务结果 schema
- 示例成功 / 失败 payload
- 前端消费边界说明（单一 API client + 类型 / adapter 生成策略）
...
**When** 团队查看 SSE 契约  
**Then** 至少能看到 `connected`、`progress`、`provider_switch`、`completed`、`failed`、`cancelled`、`heartbeat`、`snapshot` 八类事件  
