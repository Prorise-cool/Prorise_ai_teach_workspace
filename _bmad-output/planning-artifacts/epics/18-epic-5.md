## Epic 5: 主题课堂学习闭环
用户可以输入主题并按需开启联网搜索生成课堂，在等待完成后稳定浏览课堂结果中的幻灯片、讨论、白板内容，导出结果，并获得会话结束信号。  
**FRs covered:** `FR-UI-002`、`FR-CS-001`、`FR-CS-003~009`  
...
- 依赖 `Epic 7` 的联网搜索与公开资料检索能力。  
- 依赖 `Epic 10` 的课堂任务与 artifact 长期回写。  

### Cross-Epic Parallel Guardrail
- 可与 `Epic 3` 同波推进，但 `Epic 5` 只拥有课堂输入、课堂等待、课堂结果和课堂导出。  
- 不得顺手接管视频输入页、视频等待页或视频结果页。  
- 与 `Epic 7` 并行时，只允许围绕“联网搜索配置透传”对齐，不允许在未冻结边界时把资料面板直接塞进课堂主页面。  
...
- 参考成品图：`docs/03UI:UX 设计素材/001UI 设计稿/04-成品图/01-正式路由页面/06-课堂输入页/01-input.html`
- 参考成品图：`docs/03UI:UX 设计素材/001UI 设计稿/04-成品图/01-正式路由页面/07-课堂等待页/01-generating.html`
- 参考成品图：`docs/03UI:UX 设计素材/001UI 设计稿/04-成品图/01-正式路由页面/08-课堂结果页/01-classroom.html`
- 当前补充规则：课堂输入成品图只有联网开关，未单独展开最小证据范围配置；课堂结果成品图未完整展开来源抽屉入口与课后入口区；前端仍必须按 Story `5.6`、`5.7`、`5.9` 与 `Epic 7 / Epic 8` 的边界要求补足
...
Story `5.9` 与 `7.7` 需并行对齐输入配置与生成前证据增强语义。  
Story `5.10` 依赖课堂结果 schema 稳定，但不要求等待后台导出体系完成后再启动。  
...
**Given** 页面存在后续动作入口  
**When** 用户完成课堂浏览  
**Then** `Companion`、`Evidence / Retrieval` 与 `Learning Coach` 只作为侧栏或后续入口出现  
...
**Given** 用户位于 `/classroom/input`  
**When** 页面渲染完成  
**Then** 页面展示显式联网搜索开关与最小证据范围配置  
**And** 这些配置不会挤占主题输入与主 CTA 的主叙事位置  

**Given** 用户开启联网搜索并提交课堂任务  
**When** 前端创建课堂任务  
**Then** 联网搜索开关状态与当前证据范围会稳定透传给后端  
**And** 后端无需依赖隐式默认规则猜测用户是否允许调用公开资料检索  

**Given** 联网搜索不可用、结果不足或当前环境处于 mock 模式  
**When** 用户继续创建课堂任务  
**Then** 页面展示可解释提示、降级说明或 mock 样例数据  
**And** 用户仍可关闭该能力后继续生成课堂，而不是被阻断在输入页  
...
**Acceptance Criteria:**
**Given** 用户进入已完成的 `/classroom/:id`  
**When** 用户发起导出动作  
**Then** 页面返回明确的导出中、成功、失败或权限不足状态  
**And** 导出动作不会打断当前课堂主内容浏览  

**Given** 当前版本只提供基础导出能力  
**When** 系统生成导出产物  
**Then** 至少返回一种可分享文件产物，优先 `PPTX` 或等效教学分享文件  
**And** 页面明确展示当前导出类型，而不是对用户承诺不存在的多格式体系  

**Given** 导出失败、处理中或后端仍未准备完成  
**When** 用户查看结果页反馈  
**Then** 页面提供重试、稍后下载或降级说明  
**And** 失败不会污染课堂结果本身的浏览与学习链路  
