...
- [4-3-manim-修复重试上限收敛为1与worker-time-limit对齐-20260420.md](./implementation-artifacts/4-3-manim-修复重试上限收敛为1与worker-time-limit对齐-20260420.md) - Story 4.3 超时收口：patch retry 收敛为 1，并让 Dramatiq actor `time_limit` 对齐 `FASTAPI_DRAMATIQ_TASK_TIME_LIMIT_MS`，避免默认 10min 强杀
- [4-3-视频流水线占位降级与RuoYi回写修复-20260420.md](./implementation-artifacts/4-3-视频流水线占位降级与RuoYi回写修复-20260420.md) - 视频流水线补丁：分镜失败时渲染占位片段，worker 回写 RuoYi 显式携带 request_auth，发布 description 截断避免写库溢出
