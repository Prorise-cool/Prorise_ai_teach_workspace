...
| `manim_gen` | `生成动画脚本` | `26` | `42` | `8-20` | `false` |
| `tts` | `生成旁白` | `43` | `58` | `8-20` | `false` |
| `manim_fix` | `修复动画脚本` | `59` | `65` | `5-15` | `true` |
| `render` | `渲染动画` | `66` | `78` | `15-40` | `false` |
| `compose` | `合成视频` | `79` | `94` | `5-12` | `false` |
...
- `manim_gen` 与 `tts` 在实现上允许并行，但对外公开的全局 `progress` 仍必须单调递增。
- `manim_fix` 为条件阶段。若首次渲染失败，允许暴露修复态，但全局 `progress` 不得因为修复事件回退。
