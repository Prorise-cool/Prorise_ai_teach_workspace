...
const mockAsk = vi.fn().mockImplementation((req: { questionText: string; anchor: { taskId: string; seconds: number } }) =>
  Promise.resolve({
...
describe('useCompanion', () => {
  ...
  it('sets lastError on ask failure', async () => {
    mockAsk.mockRejectedValueOnce(new Error('网络错误'));
    ...
    expect(result.current.lastError).toBe('网络错误');
    expect(result.current.turns).toEqual([]);

    mockAsk.mockResolvedValue({
      turnId: `turn_${Date.now()}`,
      answerText: '回答',
      anchor: { taskId: 'task_1', seconds: 0 },
      whiteboardActions: [],
      sourceRefs: [],
      persistenceStatus: 'complete_success',
      contextSourceHit: 'redis',
