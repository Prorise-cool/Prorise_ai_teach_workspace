 ...
 *
 * 我们暂未引入 motion/react，使用纯 CSS transition + 条件渲染替代
 * Framer Motion AnimatePresence —— 视觉一致，无需新增依赖。
 */
...
interface CanvasAreaProps extends CanvasToolbarProps {
  ...
  /** 白板叠层渲染函数（可选） */
  readonly renderWhiteboard?: () => ReactNode;
}
...
export const CanvasArea: FC<CanvasAreaProps> = ({
  ...
  return (
    <div className="w-full h-full flex flex-col bg-muted/30 group/canvas">
      {/* 幻灯片区 —— 占据剩余空间 */}
      <div
        className={cn(
      ...
      >
        <div
          className={cn(
            'aspect-[16/9] h-full max-h-full max-w-full bg-card shadow-2xl rounded-lg overflow-hidden relative transition-all duration-700',
        ...
        >
          {/* 白板叠层 */}
          {renderWhiteboard && (
            <div className="absolute inset-0 z-[110] pointer-events-none">{renderWhiteboard()}</div>
          )}

          {/* 场景内容 */}
          {currentScene && !whiteboardOpen && renderScene && (
            <div className="absolute inset-0">{renderScene(currentScene)}</div>
          )}

          {/* 待生成场景的占位骨架 */}
          {isPendingScene && !currentScene && (
            <div className="absolute inset-0 z-[105] flex flex-col items-center justify-center bg-card transition-opacity duration-300">
              {isGenerationFailed ? (
          ...
          {/* 序号水印 */}
          {currentScene && (
            <div className="absolute top-4 right-4 text-foreground/10 font-black text-4xl pointer-events-none select-none">
              {(currentSceneIndex + 1).toString().padStart(2, '0')}
            </div>
          )}

          {/* 播放提示 —— 待播状态时的呼吸按钮（仅 slide 类型） */}
          {showPlayHint && (
            <div className="absolute inset-0 z-[102] flex items-center justify-center pointer-events-none transition-opacity duration-300">
              <div
      ...
      {!hideToolbar && (
        <CanvasToolbar
          ...
          whiteboardElementCount={whiteboardElementCount}
        />
      )}
    </div>
  );
