/**
 * 文件说明：Learning Coach 测评中间答题区（main，从 learning-assessment-page 拆分，wave-1.5 polish）。
 * 含题目栏 + 选项组 + 智能解析 + 上/下一题按钮。
 */
...
import { FallbackBanner } from '../pages/fallback-banner';
...
type AssessmentQuestionViewProps = {
  mode: AssessmentMode;
  pageTitle: string;
  exitTo: string;
  ...
  generationSource: string | undefined;
  canGoPrev: boolean;
  canGoNext: boolean;
...
export const LearningAssessmentQuestionView: FC<AssessmentQuestionViewProps> = ({
  mode,
  pageTitle,
  exitTo,
  currentQuestion,
  ...
  const currentJudge = currentQuestion
    ? judgeByQuestionId.get(currentQuestion.questionId)
    : undefined;
  ...
  return (
    <main className="flex-1 h-full overflow-y-auto custom-scroll flex flex-col relative px-1 md:px-2">
      ...
      {generationSource === 'fallback' ? <FallbackBanner mode={mode} /> : null}
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4 text-error">
                      ...
                      <RichInline
                        content={opt.text}
                        className="font-mono text-[15px] md:text-[16px] font-bold"
                      />
                    </div>
              ...
              return (
                <button
                  ...
                  className={[
                    ...
                    submitted
                      ? 'opacity-70 hover:opacity-100 hover:border-text-primary dark:hover:border-text-primary-dark'
                      : '',
          ...
          <div className="rounded-[20px] border border-bordercolor-light dark:border-bordercolor-dark bg-secondary/60 dark:bg-bg-dark p-6 shadow-sm">
            ...
            <div className="text-[14px] text-text-primary/90 dark:text-text-primary-dark/90 leading-relaxed font-medium">
              {submitState && currentQuestion ? (
                <RichBlock
                  content={currentJudge?.explanation ?? '暂无解析'}
                  placeholder=""
