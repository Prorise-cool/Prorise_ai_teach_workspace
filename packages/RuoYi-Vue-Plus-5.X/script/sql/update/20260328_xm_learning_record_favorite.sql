...
ALTER TABLE xm_learning_record
    ADD COLUMN display_title VARCHAR(255) DEFAULT NULL COMMENT '聚合卡片标题' AFTER result_type,
    ADD COLUMN source_table VARCHAR(64) NOT NULL DEFAULT 'xm_learning_record' COMMENT '来源宿主表' AFTER source_type,
    ADD COLUMN deleted_flag CHAR(1) NOT NULL DEFAULT '0' COMMENT '历史删除标记（0展示 1隐藏）' AFTER version_no;
...
ALTER TABLE xm_learning_record
    ADD UNIQUE KEY uk_xm_learning_record_user_source (user_id, source_table, source_result_id),
    ADD KEY idx_xm_learning_record_deleted_time (deleted_flag, source_time);
