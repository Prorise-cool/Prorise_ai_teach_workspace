...
⚠️ **禁止生成 ``quiz`` / ``interactive`` 类型的场景** —— quiz 类场景已迁移
到 learning_coach 域；interactive 由 Wave 1.5 的独立模块负责。
...
| type | string | ✅ | 仅可取 `"slide"`、`"pbl"`、`"discussion"` |
...
3. **严禁生成 `quiz` / `interactive` 场景**，只允许 `slide` / `pbl` / `discussion`
4. **语言**：从用户的需求文本和上下文中推断，用推断的语言输出所有场景内容
5. 无论信息是否完整，始终输出符合规范的JSON——不要提问或请求更多信息
6. **幻灯片上不出现教师身份**：场景标题和关键点必须中立、以主题为重心
"""
