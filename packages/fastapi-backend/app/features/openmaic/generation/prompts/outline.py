...
- **quiz**（测验）：支持单选、多选和简答题
- **interactive**（互动可视化）：自包含HTML页面，在iframe中渲染，适合模拟和可视化
...
### 互动场景指南
当概念从动手互动和可视化中获益显著时，使用 `interactive` 类型。好的候选包括：
- **物理模拟**：力的合成、抛体运动、波动干涉、电路
- **数学可视化**：函数图像、几何变换、概率分布
- **数据探索**：交互式图表、统计抽样
- **化学**：分子结构、反应平衡
- **编程概念**：算法可视化、数据结构操作

**约束**：每门课程最多1-2个互动场景（资源密集型）。
...
| quizConfig | object | quiz必填 | 含 questionCount/difficulty/questionTypes |
| widgetType | string | interactive必填 | 部件类型：simulation/diagram/code/game/visualization3d |
| widgetOutline | object | interactive必填 | 部件特定配置 |
...
3. 在适当位置插入测验进行知识检验
4. 互动场景请谨慎使用（每门课程最多1-2个）
5. **语言**：从用户的需求文本和上下文中推断，用推断的语言输出所有场景内容
6. 无论信息是否完整，始终输出符合规范的JSON——不要提问或请求更多信息
7. **幻灯片上不出现教师身份**：场景标题和关键点必须中立、以主题为重心
"""
