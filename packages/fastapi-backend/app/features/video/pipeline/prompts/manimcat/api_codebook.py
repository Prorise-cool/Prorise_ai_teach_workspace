"""Manim API codebook — compressed index of 300+ APIs for LLM prompt injection.

Source: ManimCat project (src/prompts/api-index.ts)
This codebook is the core reason ManimCat produces high-quality Manim code.
It gives LLMs precise knowledge of available methods, parameters, and patterns.
"""

API_INDEX = """# Manim API Index (AI, codebook-compressed)
Notes:
...
args_codebook = {
  ...
  radius: rad, quadrant: quad, other_angle: o_ang, dot: dot,
  dot_radius: d_rad, dot_distance: d_dis, dot_color: d_col, elbow: elb,
  inner_radius: i_rad, outer_radius: o_rad, mark_paths_closed: mpc,
  start_angle: s_ang, start: st, end: ed, num_components: n_comp,
  arc_center: a_ctr, vertices: verts, arc_config: a_cfg, mobject: mobj,
  scale_factor: sc_f, start_anchor: sa, start_handle: shd,
  end_handle: ehd, end_anchor: ea, main_shape: m_sh, subject: subj,
  clip: clip, point: pt, width: wid, height: hgt, vmobjects: vms,
  vertex_groups: v_gps, grid_xstep: gx, grid_ystep: gy, n: n,
  num_vertices: n_v, density: dns, length: len, corner_radius: c_rad,
  aspect_ratio: asp_r, side_length: sd_l, thickness: thk,
  base_radius: b_rad, resolution: res, direction: dir, show_base: sh_b,
  v_range: v_r, u_min: u_min, checkerboard_colors: cb_cols,
  show_ends: sh_e, edge_length: e_len, dimensions: dims,
  vertex_coords: v_coords, faces_list: f_list, faces_config: f_cfg,
  graph_config: g_cfg, center: ctr, u_range: u_r, major_radius: mj_rad,
  minor_radius: mn_rad, items: items, dot_scale_factor: d_scf,
  tex_environment: tex_env, code_file: c_file, code_string: c_str,
  language: lang, formatter_style: fmt_style, tab_width: tab_w,
  add_line_numbers: aln, line_numbers_from: lnf, background: bg,
  background_config: bg_cfg, paragraph_config: p_cfg, text: text,
  font_size: fs, line_spacing: ls, font: font, slant: slant,
  weight: wgt, justify: just, gradient: grad, should_center: sh_ctr,
  disable_ligatures: dis_lig, warn_missing_font: wmf,
  tex_strings: texs, arg_separator: arg_sep,
  substrings_to_isolate: s2i, tex_to_color_map: t2cm,
  tex_string: tex1, organize_left_to_right: oltr,
  tex_template: tex_tpl, t2c: t2c, t2f: t2f, t2g: t2g, t2s: t2s,
  t2w: t2w, use_svg_cache: usc, text_parts: t_parts,
  include_underline: inc_ul, match_underline_width_to_text: muwtt,
  underline_buff: ul_buf, alignment: align, x_range: x_r,
  y_range: y_r, z_range: z_r, x_length: x_l, y_length: y_l,
  z_length: z_l, axis_config: ax_cfg, x_axis_config: xax_cfg,
  y_axis_config: yax_cfg, z_axis_config: zax_cfg, tips: tips,
  values: vals, bar_names: b_names, bar_colors: b_cols, bar_width: b_w,
  bar_fill_opacity: b_fo, bar_stroke_width: b_sw, edges: eds,
  labels: lbls, label_fill_color: lfc, layout: layout,
  layout_scale: lay_s, layout_config: lay_cfg, vertex_type: v_type,
  vertex_config: v_cfg, vertex_mobjects: v_mobs, edge_type: e_type,
  partitions: parts, root_vertex: r_vert, edge_config: e_cfg,
  unit_size: u_size, include_ticks: inc_ticks, tick_size: tick_s,
  numbers_with_elongated_ticks: nwet, longer_tick_multiple: ltm,
  exclude_origin_tick: eot, rotation: rot, include_tip: inc_tip,
  tip_width: tip_w, tip_height: tip_h, tip_shape: tip_shape,
  include_numbers: inc_nums, label_direction: l_dir,
  label_constructor: l_ctor, scaling: scaling,
  line_to_number_buff: ltnb, decimal_number_config: dec_cfg,
  numbers_to_exclude: n_ex, numbers_to_include: n_in,
  radius_max: r_max, size: size, radius_step: r_step,
  azimuth_step: az_step, azimuth_units: az_units,
  azimuth_compact_fraction: az_cf, azimuth_offset: az_off,
  azimuth_direction: az_dir, azimuth_label_buff: az_lb,
  azimuth_label_font_size: az_lfs, radius_config: r_cfg,
  background_line_style: bls, faded_line_style: fls,
  faded_line_ratio: flr, z_normal: z_n, num_axis_pieces: nap,
  light_source: l_src, depth: depth, gloss: gloss,
  tip_length: tip_l, sharpness: shrp, num_dashes: n_d,
  num_decimal_places: ndp, include_sign: inc_s,
  group_with_commas: gwc, unit: unit, data: data,
  row_labels: r_lbls, col_labels: c_lbls,
  include_outer_lines: iol, function: func, t_range: t_r,
  use_smoothing: u_sm, amplitude: amp, time_width: t_w,
  lag_ratio: lag_r, n_times: n_t, flash_radius: fl_r,
  line_length: ln_l
}
...
SOUL_INDEX = """# Manim Soul Index (Compressed)

### 1. Animation Codebook (The 'How it moves')
anim_codebook = {
  Create: cr, Write: wr, FadeIn: fi, FadeOut: fo,
  Transform: tr, ReplacementTransform: rttr,
  TransformMatchingTex: tmtex, TransformMatchingShapes: tmsh,
  FadeTransform: ftr, TransformFromCopy: tfc,
  MoveAlongPath: map, Rotate: rot_anim, ScaleInPlace: sip,
  GrowFromCenter: gfc, ShrinkToCenter: stca,
  Indicate: ind, Circumscribe: circ, Flash: flsh,
  FocusOn: fon, ShowPassingFlash: spf, Wiggle: wgl,
  ApplyWave: aw, LaggedStart: ls, Succession: succ,
  AnimationGroup: agp
}

### 2. Rate Function Codebook (The 'Feel')
rate_codebook = {
  smooth: sm, linear: ln, double_smooth: dsm,
  ease_in_sine: eis, ease_out_sine: eos, ease_in_out_sine: eios,
  ease_in_out_quad: eioq, ease_in_cubic: eic, ease_out_cubic: eoc,
  rush_from: rf, rush_into: ri, slow_into: si,
  there_and_back: tab, there_and_back_with_pause: tabp,
  lingering: ling, wiggle: wig, running_start: rs
}

### 3. Logic & Updater Codebook (The 'Brain')
logic_codebook = {
  ValueTracker: vt, DecimalNumber: dn,
  add_updater: au, remove_updater: ru, always_redraw: alr,
  get_value: gv, set_value: sv, increment_value: iv
}

### 4. Camera & 3D Codebook (The 'Eyes')
camera_codebook = {
  set_camera_orientation: sco, move_camera: mc,
  add_fixed_in_frame_mobjects: afif,
  phi: p, theta: t, gamma: g, zoom: z, focal_distance: fd
}
...
const_codebook = {
  ORIGIN: O, UP: U, DOWN: D, LEFT: L, RIGHT: R,
...
SHARED_SPECIFICATION = """## Shared Specification
...
def build_api_index_module() -> str:
    """Build the full API index module for prompt injection."""
    return f"""## API Index
