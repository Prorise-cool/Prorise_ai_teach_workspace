...
from .c2v_utils import eva_video_list, extract_answer_from_response
...
class VideoEvaluator:
    ...
    def evaluate_video(
        self, video_path: str, knowledge_point: str, log_id: str = None
    ) -> EvaluationResult:
        """
    ...
    def _evaluate_video_batch_sequential(
        self, video_list: List[Dict[str, Any]], log_id: str = None
    ) -> List[EvaluationResult]:
        results = []
        ...
        for i, video_info in enumerate(video_list):
            ...
            if not knowledge_point:
                print(
                    f"Warning: Video {i + 1} is missing knowledge_point information, which may affect evaluation accuracy"
                )
            ...
            result = self.evaluate_video(
                ...
                log_id=f"{log_id}_video_{i + 1}" if log_id else None,
            )
    ...
    def _evaluate_video_batch_parallel(
        ...
        print(
            f"Starting parallel evaluation of {len(video_list)} videos using {max_workers} worker threads..."
        )
        ...
        def evaluate_single_video(index: int, video_info: Dict[str, Any]) -> tuple:
            ...
            if not knowledge_point:
                with self._progress_lock:
                    print(
                        f"Warning: Video {index + 1} is missing knowledge_point information, which may affect evaluation accuracy"
                    )
            ...
            try:
                result = self.evaluate_video(
                    video_path=video_path,
                    knowledge_point=knowledge_point,
                    log_id=f"{log_id}_video_{index + 1}" if log_id else None,
                )
                return index, result, None
            except Exception as e:
                error_result = self._create_error_result(
                    f"Parallel evaluation error: {str(e)}"
                )
                return index, error_result, str(e)

        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_index = {
                executor.submit(evaluate_single_video, i, video_info): i
                for i, video_info in enumerate(video_list)
            }
            for future in as_completed(future_to_index):
                try:
                    ...
                    with self._progress_lock:
                        ...
                        print(
                            f"Completed {completed_count}/{len(video_list)} "
                            f"(Time: {elapsed_time:.1f}s, ETA: {eta:.1f}s)"
                        )

                        if error:
                            print(
                                f"Warning: Video {index + 1} evaluation encountered an error: {error}"
                            )
                        else:
                            video_path = video_list[index].get("path", "")
                            knowledge_point = video_list[index].get(
                                "knowledge_point", ""
                            )
                            print(
                    ...
                    with self._progress_lock:
                        print(
                            f"Warning: Error processing future result for Video {index + 1}: {str(e)}"
                        )
    ...
    def _parse_evaluation_response(self, response: str) -> EvaluationResult:
        ...
        try:
            ...
            if json_match:
                ...
                visual_consistency = float(
                    data.get("visual_consistency", {}).get("score", 0)
                )
    ...
    def generate_evaluation_report(
        self, results: List[EvaluationResult], output_path: str = None
    ) -> str:
        if not results:
        ...
        avg_scores = {
            ...
            "visual_consistency": sum(r.visual_consistency for r in results)
            / total_videos,
...
def evaluate_main():
    ...
    batch_results = evaluator.evaluate_video_batch(
        video_list, max_workers=3, use_parallel=True
    )
