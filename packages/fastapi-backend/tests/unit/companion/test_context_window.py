...
class TestContextWindow:
    ...
    def test_save_and_load_roundtrip(self) -> None:
        ...
        cw.save("s1", window)
        store.set_runtime_value.assert_called_once_with(
            f"{CONTEXT_KEY_PREFIX}s1",
            __import__("json").dumps(window, ensure_ascii=False),
            ttl_seconds=cw.context_ttl_seconds,
        )
    ...
    def test_window_truncation(self) -> None:
        ...
        store = _make_store(existing)
        cw = ContextWindow(store, max_rounds=max_rounds, recent_rounds_to_keep=recent_rounds_to_keep)
        result = cw.append_turn(
            "s1",
            turn_id="t_new",
            question_text="新问题",
