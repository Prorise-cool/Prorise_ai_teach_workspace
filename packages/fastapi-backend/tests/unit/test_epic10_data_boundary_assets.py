...
SQL_LEARNING_RESULT = REPO_ROOT / "packages/RuoYi-Vue-Plus-5.X/script/sql/update/20260328_xm_learning_result.sql"
...
def test_story_10_1_assets_exist() -> None:
    ...
    assert SQL_LEARNING_RESULT.exists()
...
def test_learning_path_doc_fields_align_with_learning_result_sql() -> None:
    baseline_text = DOC_BASELINE.read_text(encoding="utf-8")
    learning_sql_text = SQL_LEARNING_RESULT.read_text(encoding="utf-8")
    learning_path_section = baseline_text.split("### `xm_learning_path`", maxsplit=1)[1].split(
        "### `xm_learning_record`",
        maxsplit=1
    )[0]
    ...
    for field_name in required_doc_fields:
        assert field_name in learning_path_section
        assert field_name in learning_sql_text

    for field_name in removed_legacy_fields:
        assert field_name not in learning_path_section
